...
public class BeneficiaryService(
    ...
    public async Task<ResultData<IEnumerable<BeneficiaryDto>?>> GetByAccount(Guid accountId) =>
        await GetAllAsync<BeneficiaryDto>($"accounts/{accountId}/beneficiaries");
    
    // GET a beneficiary by Id 
    public async Task<ResultData<BeneficiaryDto?>> GetById(Guid beneficiaryId) =>
        await GetAsync<BeneficiaryDto>($"beneficiaries/{beneficiaryId}");

    // POST: Insert a beneficiary
    public async Task<ResultData<BeneficiaryDto?>> Post(Guid accountId, BeneficiaryDto beneficiaryDto) =>
